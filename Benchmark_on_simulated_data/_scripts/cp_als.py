"""CP Decomposition via Alternating Least Squares. Modified to allow penalization with Laplacian"""
...
def cp_als(  # noqa: PLR0912,PLR0913,PLR0915
    ...
    """Compute CP decomposition with alternating least square and Laplacian penalization.
    ...
    input_tensor:
        Tensor to decompose
    Lapmat:
		Laplacian matrix for the regularization A.T @ L @ A
    rank:
    ...
    if isinstance(init, ttb.ktensor):
        ...
        for n in range(N):
            factor_matrices.append(
                rng.uniform(0, 1, (input_tensor.shape[n], rank))
            )
    ...
    for iteration in range(maxiters):
            ...
            if (Y == 0).all():
                ...
                if n != 0:
                    try:
                        ...
                        Unew = rng.uniform(0, 1, Unew.shape)
                else:
                     Unew = solve_sylvester(reg_lam*Lapmat, Y, Unew)
        ...
        penalty = reg_lam * np.trace(U[0].T @ Lapmat @ U[0])
        if normX == 0:
        ...
        obj = normresidual**2 + penalty
        fitchange = np.abs(fitold - fit)
        objchange = np.abs(objold-obj)

        # Check for convergence
        if (iteration > 0) and (round(obj, 12) == 0.0 or objchange < stoptol*obj):
            flag = 0
        else:
            flag = 1

        if (printitn > 0) and ((divmod(iteration, printitn)[1] == 0) or (flag == 0)):
            rel_objchange = 0.0 if round(objchange, 12) == 0.0 else objchange / obj
            print(f" Iter {iteration}: f = {fit:e} f-delta = {fitchange:7.1e} obj = {obj:e} rel-objchange = {rel_objchange:7.1e}")
